<template>
  <div class="app" @dragover.prevent="onDrag">
    ...
    <input class="title" type="text" placeholder="Comparison Title" v-model="title">

    <div class="mass-rename">
      <strong>Mass Rename</strong>
      <div>
    ...
    <div v-if="false">
      ...
      <span class="batch-move" v-for="c in columns">Column {{ c }}</span>
    </div>
    ...
    <draggable v-model="images">
      <div class="row" v-for="r in rows">
        <div class="info">
          <div class="number">
            <ScalingText>{{ r }}</ScalingText>
          </div>
          <div class="remove" @click="removeRow(r-1)">
            <ScalingText>REMOVE</ScalingText>
          </div>
        </div>
    ...
    <div class="control">
      <div @click.prevent="addRow">Add Row</div>
      <div @click.prevent="addColumn">Add Column</div>
      <div @click.prevent="removeColumn">Remove Column</div>
    </div>

    <div class="actions">
      <div class="show-preview" @click.prevent="maybeShowPreview">
        <ScalingText>Preview Comparison</ScalingText>
      </div>
      <div class="submit" @click.prevent="submit">
        <ScalingText>Submit Comparison</ScalingText>
      </div>
    </div>

    <div class="hint">Hint: You can drag & drop multiple images at once!</div>
    ...
    <div
      ...
      v-show="magic"
      @drop.prevent="onDrop"
      @dragleave.prevent="magic = false"
      @dragend.prevent
    >
      <span>
        Dragging more than one file at a time doesn't work with the manual UI.
    ...
    <div class="error" v-show="error">
      <h1>ERROR</h1>
      <span>{{ error }}</span>
      <div class="ok" @click.prevent="error = ''">OK</div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["rows", "columns"])
  },
  methods: {
    ...mapMutations(["addRow", "removeRow", "addColumn", "removeColumn"]),
      ...
      for (let i = 0; i < this.images.length; i++) {
        for (let j = 0; j < this.images[i].length; j++) {
          if (this.images[i][j].file === null) {
            this.error =
              "Some image slots are missing images. Please fill all slots or remove rows or columns as needed before submitting.";
            return;
          }
        }
      }
    ...
    submit: function() {
      ...
      fetch("https://diff.theiahd.nl/check_images", {
        ...
        body: JSON.stringify(Object.keys(files))
      })
        .then(response => {
    ...
    onDrop: function(e) {
      ...
      Promise.all([MassImageCompare(files), Promise.all(shaPromises)]).then(
        data => {
          ...
          if (images.length >= 2) {
            columns = columns.map((name, col) => {
              // Use map so that we can easily return from nested loops
